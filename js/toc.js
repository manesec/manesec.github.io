// https://github.com/vincentlaucsb/js-toc/blob/master/dist/toc.js
// Mod by @manesec
...
function makeListItem(level,href, text) {
    // Generate a bullet point containing a link
    var link = document.createElement('a');
    link.setAttribute('href', href);
    link.innerHTML = text;
    link.classList.add('level-'+level.toString());
...
function makeList(listParams) {
    ...
    for (var i in headers) {
        ...
        //console.log(tocItem);
        var link = makeListItem(tocItem.level ,"#" + tocItem.id, tocItem.name);
        // Add link text
        if (currentLevel) {
